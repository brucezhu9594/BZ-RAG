...
定完之后两个规模都真跑了一遍验证（2026-09-11）：CI 上 master 全量 `contextual_precision`
`pass_rate` 实测 **1.000**（n=48，阈值 0.85），本机 smoke 实测 **1.000**（n=3）。smoke 这个
结果是必然而非侥幸——那条双峰 case 无论落 0.5 还是 1.0 都满足 `score >= 0.5`，只有某条掉到
0.0 才会挂，而历史 30 次观测里没出现过。这正是 `pass_rate` 比 `average` 稳的地方：`average`
会被 0.5 和 1.0 的差别晃动，`pass_rate` 不会。
